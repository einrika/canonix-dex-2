...
const holdersHandler = async (req, res) => {
    ...
    const { contract_address, address, page = 0 } = req.query || {};
    const addr = contract_address || address;

    if (!addr || !isValidPaxiAddress(addr)) {
        return sendResponse(res, false, null, 'Invalid contract address', 400);
    }

    const cacheKey = `holders_${addr}_p${page}`;
    const cachedData = getCached(cacheKey);
    if (cachedData) return sendResponse(res, true, cachedData);

    try {
        const apiUrl = `https://explorer.paxinet.io/api/prc20/holders?contract_address=${addr}&page=${page}`;
