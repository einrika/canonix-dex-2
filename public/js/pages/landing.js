...
export const initLandingPage = async () => {
    ...
    if (window.initGlobalMarketAI) window.initGlobalMarketAI();

    window.addEventListener('paxi_token_list_updated', (event) => {
        const data = event.detail;
        if (data && data.contracts && marketTokens.length > 0) {
            data.contracts.forEach(c => {
                const index = marketTokens.findIndex(t => t.address === c.contract_address);
                if (index !== -1) {
                    marketTokens[index] = window.processTokenDetail(c.contract_address, c);
                }
            });
            renderMarketGrid();
        }
...
export const setMarketFilter = function(type, btn) {
    ...
    if (type === 'nonpump') {
        ...
        renderMarketSubTabs(); loadMarketData('nonpump');
    } else {
...
export const loadMoreMarket = async function() {
    ...
    finally { marketIsFetching = false; if (btn) { btn.disabled = false; btn.innerHTML = 'LOAD MORE'; } }
};
...
export function renderMarketGrid() {
    ...
    grid.innerHTML = filtered.map(t => {
        ...
        return `<a href="trade.html?token=${t.address}" class="bg-surface border-4 border-card p-4 md:p-6 shadow-brutal hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all group block relative overflow-hidden"><div class="flex items-center gap-3 md:gap-4 mb-6 md:mb-8"><div class="relative flex-shrink-0">${logoUrl ? `<img src="${logoUrl}" class="w-10 h-10 md:w-12 md:h-12 border-2 border-card group-hover:rotate-6 transition-transform shadow-brutal-sm object-cover" onerror="this.classList.add('hidden'); this.nextElementSibling.classList.remove('hidden');"><div class="hidden w-10 h-10 md:w-12 md:h-12 bg-meme-yellow border-2 border-card flex items-center justify-center text-lg font-display text-black shadow-brutal-sm">${t.symbol.charAt(0)}</div>` : `<div class="w-10 h-10 md:w-12 md:h-12 bg-meme-yellow border-2 border-card flex items-center justify-center text-lg font-display text-black shadow-brutal-sm">${t.symbol.charAt(0)}</div>`}</div><div class="flex-1 overflow-hidden"><div class="flex items-center gap-1.5 flex-wrap"><span class="font-display text-lg md:text-2xl text-primary-text tracking-tighter uppercase italic break-all">${t.symbol}</span>${t.verified ? `<i class="fas fa-check-circle text-meme-cyan text-xs md:text-sm flex-shrink-0"></i>` : ''}</div><div class="text-[8px] md:text-[10px] text-meme-cyan font-mono font-bold uppercase tracking-widest">PRC-20 ASSET</div></div></div><div class="space-y-3 md:space-y-4 relative z-10"><div><div class="text-[8px] md:text-xs text-secondary-text font-mono font-bold uppercase tracking-widest mb-1 italic">Price</div><div class="text-lg md:text-2xl font-display text-primary-text italic tracking-tight truncate">${t.price_paxi.toFixed(8)} <span class="text-meme-yellow text-xs">PAXI</span></div></div><div class="flex justify-between items-center pt-3 md:pt-4 border-t-2 border-card"><div class="px-2 md:px-3 py-0.5 md:py-1 border-2 border-card font-display text-[10px] md:text-base ${colorClass}">${t.price_change_24h >= 0 ? '+' : ''}${change}%</div><div class="text-right"><div class="text-[7px] md:text-[8px] text-muted-text font-mono font-bold uppercase tracking-widest italic">Volume</div><div class="text-[10px] md:text-base font-display text-meme-cyan italic tracking-tight">${vol}</div></div></div><div class="text-[8px] text-secondary-text font-mono mt-0.5" title="${t.address}">${t.address.slice(0, 6)}....${t.address.slice(-6)}</div></div></a>`;
    }).join('');
