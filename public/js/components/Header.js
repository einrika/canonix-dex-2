...
import { shortenAddress, formatBalance } from '../core/utils.js';
...
export const Header = {
            ...
            return `
                <div class="max-w-7xl mx-auto h-14 flex justify-between items-center px-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-2xl font-display tracking-widest text-primary-text italic uppercase">CANONIX</span>
                    </div>
    ...
    init: (container, props = {}) => {
        ...
        const toggleMobileSidebar = () => {
             // We will need a global way to trigger sidebar toggle or use State
             window.toggleMobileSidebar?.();
        };

        container.querySelector('#mobile-sidebar-toggle')?.addEventListener('click', toggleMobileSidebar);
        ...
        State.subscribe('wallet', (wallet) => {
            ...
            const balanceEl = container.querySelector('#walletBalance');

            if (wallet && wallet.address) {
                if (connectBtn) connectBtn.classList.add('hidden');
                if (walletInfo) walletInfo.classList.remove('hidden');
                if (addrEl) addrEl.textContent = shortenAddress(wallet.address);
                if (balanceEl) balanceEl.textContent = `${formatBalance(wallet.balance || 0)} PAXI`;
            } else {
