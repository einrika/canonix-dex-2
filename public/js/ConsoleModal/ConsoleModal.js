...
export const logSuccess = function(...args) {
...
export const toggleConsoleModal = function() {
    const modal = document.getElementById('consoleModal');
    if (!modal) return;
...
export const clearConsoleLogs = function() {
...
export const ConsoleModalLogic = (container) => {
    ...
    container.querySelector('#close-console-modal')?.addEventListener('click', () => toggleConsoleModal());
    container.querySelector('#purge-console')?.addEventListener('click', () => clearConsoleLogs());
};

originalConsole.log('🚀 SYSTEM UPLINK ESTABLISHED');
originalConsole.log('CONSCIOUSNESS ACTIVE');

window.toggleConsoleModal = toggleConsoleModal;
