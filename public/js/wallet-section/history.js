...
window.WalletHistory = {
    ...
    async loadHistory(forceRefresh = false) {
        ...
        const addr = activeWallet?.address || window.wallet?.address || window.selectedAddress || window.currentAddress;

        if (addr && !window.historyIsLoading) {
            if (forceRefresh || (window.txHistory.length === 0 && !window.historyIsEnd)) {
                ...
                try {
                    ...
                    if (Array.isArray(result)) {
                        ...
                        window.historyIsEnd = result.length < 10;
                    }
                } catch (e) {
        ...
        window.renderTransactionHistory?.(window._lastHistoryContainer);
    }
};
...
window.renderTransactionHistory = function(customContainerId) {
    if (customContainerId) window._lastHistoryContainer = customContainerId;

    let cont = customContainerId ? document.getElementById(customContainerId) : null;

    if (!cont) {
        const tabContent = document.getElementById('tabContent');
        const sidebarHistory = document.getElementById('history-container');
        const historyTabBtn = document.getElementById('tab-history');
        if (historyTabBtn && historyTabBtn.classList.contains('active')) {
            cont = tabContent;
        } else {
            cont = sidebarHistory || tabContent;
        }
    }
