...
import { fetchDirect, shortenAddress, formatBalance, addClass, removeClass } from '../../core/utils.js';
import { APP_CONFIG } from '../../core/config.js';

// ===== GLOBAL HOLDERS STATE =====
export let holdersList = [];
export let holdersPage = 1;
export let isFetchingHolders = false;
export let holdersCache = {};

// ===== SHOW HOLDERS MODAL =====
export const showHolders = function() {
    ...
    if (tabBtn) {
        ...
        else { holdersPage = 1; loadTokenHolders(); }
    } else {
        removeClass('holdersModal', 'hidden'); holdersPage = 1; loadTokenHolders();
    }
};

export const hideHolders = function() { addClass('holdersModal', 'hidden'); };

// ===== LOAD HOLDERS DATA =====
export const loadTokenHolders = async function(isRefresh = true) {
    const container = document.getElementById('tabContent');
    if (!container) return;
    if (isRefresh) { holdersPage = 1; holdersCache = {}; }
    if (holdersPage === 1) {
    ...
    try { await fetchTokenHolders(holdersPage); }
    catch (e) {
...
// ===== FETCH TOKEN HOLDERS =====
export const fetchTokenHolders = async function(page) {
    if (isFetchingHolders) return;
    isFetchingHolders = true;
    const cacheKey = `${window.currentPRC20}_p${page}`;
    if (holdersCache[cacheKey] && (Date.now() - holdersCache[cacheKey].time < 60000)) {
        holdersList = holdersCache[cacheKey].data;
        renderHoldersTable(holdersList, page);
        isFetchingHolders = false; return;
    }
    try {
        const url = `${APP_CONFIG.BACKEND_API}/api/holders?address=${window.currentPRC20}&page=${page - 1}`;
        ...
        holdersList = list;
        holdersCache[cacheKey] = { data: list, time: Date.now() };
        renderHoldersTable(holdersList, page);
    } catch (e) { console.error("Holders API error:", e); } finally { isFetchingHolders = false; }
};

// ===== RENDER HOLDERS TABLE =====
export const renderHoldersTable = function(list, page) {
    const container = document.getElementById('tabContent');
    if (!container) return;
    const limit = APP_CONFIG.ITEMS_PER_PAGE || 30;
    ...
    const headers = `<tr class="font-display text-base text-secondary-text uppercase italic border-b-2 border-card"><th class="p-4">RANK</th><th class="p-4">ADDRESS</th><th class="p-4 text-right">BAG</th><th class="p-4 text-right">SHARE</th></tr>`;
    let html = `<div class="overflow-x-auto no-scrollbar"><table class="w-full text-left border-collapse"><thead class="bg-surface sticky top-0 z-10">${headers}</thead><tbody class="divide-y divide-black/50">`;
    for (let idx = 0; idx < list.length; idx++) {
        ...
        const shortAddr = typeof addr === 'string' ? shortenAddress(addr, 7) : 'Unknown';
        const fullAddr = typeof addr === 'string' ? addr : '';
        let balanceDisplay = bal; if (typeof bal === 'number') balanceDisplay = formatBalance(bal, 6);
        else if (typeof bal === 'string' && bal !== '-' && bal !== 'Loading...') {
            const numBal = parseFloat(bal); if (!isNaN(numBal)) balanceDisplay = formatBalance(numBal, 6);
        }
...
export const loadMoreHolders = async function() {
    if (isFetchingHolders) return;
    ...
    holdersPage++; await fetchTokenHolders(holdersPage);
};

window.showHolders = showHolders;
window.hideHolders = hideHolders;
window.loadTokenHolders = loadTokenHolders;
window.loadMoreHolders = loadMoreHolders;
