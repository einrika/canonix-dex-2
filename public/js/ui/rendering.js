...
window.showTransactionDetailModal = async function(hash) {
    ...
    modal.className = 'fixed inset-0 bg-surface/90 z-[600] flex items-center justify-center p-4 animate-fade-in';
    ...
    let messagesHtml = body.messages.map((m, i) => {
        ...
        return `<div class="p-3 bg-card/50 rounded-xl border border-border/50 mb-2">
            ...
            <pre class="text-[9px] font-mono text-secondary-text overflow-x-auto">${JSON.stringify(m, null, 2)}</pre>
        </div>`;
    }).join('');
    ...
    modal.innerHTML = `
        <div class="bg-surface border border-border w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
            <div class="p-6 border-b border-border flex justify-between items-center bg-card/30">
                ...
                <button onclick="this.closest('.fixed').remove()" class="text-secondary-text hover:text-primary-text transition-all"><i class="fas fa-times"></i></button>
            </div>
            <div class="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
                <div class="grid grid-cols-2 gap-4">
                    <div class="p-4 bg-bg rounded-2xl border border-border">
                        <div class="text-[9px] text-secondary-text uppercase font-black mb-1">Status</div>
                        <div class="text-sm font-black ${statusColor}">${statusText}</div>
                    </div>
                    <div class="p-4 bg-bg rounded-2xl border border-border">
                        <div class="text-[9px] text-secondary-text uppercase font-black mb-1">Block Height</div>
                    <div class="text-sm font-black text-primary-text">${tx.height || tx.block || "-"}</div>
                    </div>
                </div>

                <div class="p-4 bg-bg rounded-2xl border border-border">
                    <div class="text-[9px] text-secondary-text uppercase font-black mb-1">Transaction Hash</div>
                ...
                <div class="p-4 bg-bg rounded-2xl border border-border">
                    <div class="text-[9px] text-secondary-text uppercase font-black mb-1">Timestamp</div>
                    <div class="text-xs text-primary-text">${timeStr}</div>
                </div>

                <div>
                    <h4 class="text-[10px] font-black text-secondary-text uppercase tracking-widest mb-3">Messages</h4>
                    ${messagesHtml}
                </div>

                <div class="p-4 bg-bg rounded-2xl border border-border">
                    <div class="text-[9px] text-secondary-text uppercase font-black mb-1">Gas Used / Wanted</div>
                    <div class="text-xs text-primary-text">${tx.gas_used || tx.gasUsed || "-"} / ${tx.gas_wanted || tx.gasWanted || "-"}</div>
                </div>
