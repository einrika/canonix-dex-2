// ============================================
// COMMON.JS - UI Interactions & Modal Management
// ============================================

import { removeClass, addClass, setText, setHtml, formatPrice, formatAmount, shortenAddress } from '../core/utils.js';
import { APP_CONFIG } from '../core/config.js';
import { fetchDirect, getPRC20Balance } from '../core/api.js';

// ===== SHOW/HIDE CONNECT MODAL =====
export const showConnectModal = function() {
    removeClass('connectModal', 'hidden');
    addClass('connectModal', 'flex');
};

export const hideConnectModal = function() {
    addClass('connectModal', 'hidden');
    removeClass('connectModal', 'flex');
};

// ===== DONATION MODAL =====
export const showDonationModal = function() {
    removeClass('donationModal', 'hidden');
};

export const hideDonationModal = function() {
    addClass('donationModal', 'hidden');
};

// ===== FAQ MODAL =====
export const showFAQ = function() {
    removeClass('faqModal', 'hidden');
};

export const hideFAQ = function() {
    addClass('faqModal', 'hidden');
};
...
export const toggleFAQ = function(element) {
    ...
    document.querySelectorAll('.faq-answer').forEach(el => {
        if (el !== answer) {
            ...
            if (el.previousElementSibling) {
                removeClass(el.previousElementSibling, 'active');
            }
        }
    });
    ...
    if (!answer.classList.contains('hidden')) {
        ...
        addClass(answer, 'open');
        addClass(element, 'active');
    }
};

// ===== TOKEN SELECTOR MODAL =====
export const showTokenSelector = function() {
    removeClass('tokenModal', 'hidden');
    addClass('tokenModal', 'flex');
    renderTokenList();
};

export const hideTokenSelector = function() {
    addClass('tokenModal', 'hidden');
    removeClass('tokenModal', 'flex');
};

export const filterTokens = function() {
    renderTokenList(document.getElementById('tokenSearch')?.value);
};

export const renderTokenList = function(filter = '') {
...
// ===== UPDATE DASHBOARD =====
export const updateDashboard = function(detail) {
    if (!detail) return;
    const address = window.currentPRC20;

    setText('selectedPair', `${detail.symbol}/PAXI`);
    setText('currentPrice', formatPrice(detail.price_paxi) + ' PAXI');
    ...
    setText('mcapVal', formatAmount(detail.market_cap) + ' PAXI');
    setText('liqVal', formatAmount(detail.liquidity) + ' PAXI');
    setText('volVal', formatAmount(detail.volume_24h) + ' PAXI');
    ...
    setText('resPaxi', formatAmount(detail.reserve_paxi / 1000000, 2));
    setText('resToken', formatAmount(detail.reserve_prc20 / Math.pow(10, detail.decimals), 2));
    setText('resTokenLabel', detail.symbol + ' STOCK');
    setText('holderCount', detail.holders.toLocaleString());

    setText('caAddr', address);
    setText('totalSupply', formatAmount(detail.total_supply_num, 0));

    const minter = document.getElementById('minterStatus');
    if (minter) {
        setText(minter, detail.minting_disabled ? 'Freezing and minting have been revoked' : 'Freezing and minting have not been revoked');
    ...
    if (verif) {
        setText(verif, detail.official_verified ? 'OFFICIAL VERIFIED' : 'NOT VERIFIED');
    ...
    if (descCard) {
        ...
        setText('tokenDesc', detail.description || 'NO DESCRIPTION AVAILABLE.');
        setText('mktAddr', detail.marketing_wallet || 'N/A');
    }

    const logoEl = document.getElementById('tokenLogo');
    if (logoEl) {
        if (detail.logo) {
            setHtml(logoEl, `<img src="${detail.logo}" class="w-full h-full object-cover">`);
        } else {
            setHtml(logoEl, `<span class="text-black font-display text-3xl">${detail.symbol.charAt(0)}</span>`);
        }
    }
    ...
    setText('recvTokenSymbol', detail.symbol);
    updateTradeBalances();
};
...
export const setTradeType = function(type) {
    tradeType = type;
    ...
    if (type === 'buy') {
        ...
        setText('payTokenSymbol', 'PAXI');
        setText('recvTokenSymbol', window.currentTokenInfo?.symbol || 'TOKEN');
    } else {
        ...
        setText('payTokenSymbol', window.currentTokenInfo?.symbol || 'TOKEN');
        setText('recvTokenSymbol', 'PAXI');
    }
    updateTradeBalances();
};

export const updateTradeBalances = async function() {
    ...
    try {
        const balData = await fetchDirect(`${APP_CONFIG.BACKEND_API}/api/paxi-balance?address=${walletAddress}`);
        ...
        if (window.currentPRC20) {
            ...
            const bal = await getPRC20Balance(walletAddress, window.currentPRC20);
        ...
        if (window.tradeType === 'buy') {
            if (payBalEl) { setText(payBalEl, paxiAmount.toFixed(4)); payBalEl.setAttribute('data-raw', paxiRaw); }
            if (recvBalEl) { setText(recvBalEl, prc20Amount.toFixed(4)); recvBalEl.setAttribute('data-raw', prc20Raw); }
        } else {
            if (payBalEl) { setText(payBalEl, prc20Amount.toFixed(4)); payBalEl.setAttribute('data-raw', prc20Raw); }
            if (recvBalEl) { setText(recvBalEl, paxiAmount.toFixed(4)); recvBalEl.setAttribute('data-raw', paxiRaw); }
        }

        setText('walletAddrShort', shortenAddress(walletAddress));
        setText('walletBalance', paxiAmount.toFixed(2) + ' PAXI');
        removeClass('walletInfo', 'hidden');
        setText('mobileWalletBalance', paxiAmount.toFixed(2) + ' PAXI');
        removeClass('mobileWalletInfo', 'hidden'); addClass('mobileConnectBtn', 'hidden');

        const sidebarPaxi = document.getElementById('sidebar-paxi-bal');
        if (sidebarPaxi) setText(sidebarPaxi, paxiAmount.toFixed(2));
        const portfolioUSD = document.getElementById('portfolio-usd');
        if (portfolioUSD) { const usdValue = paxiAmount * (window.paxiPriceUSD || 0.05); setText(portfolioUSD, `$${usdValue.toFixed(2)} USD`); }
    } catch (e) { console.error('❌ Balance update failed:', e); }
};

export const copyAddrText = function() {
    if (!window.currentPRC20) return;
    navigator.clipboard.writeText(window.currentPRC20);
};

export const copyMktAddr = function() {
...
export const showSlippageModal = function() {
    removeClass('slippageModal', 'hidden');
    if (window.slippage) window.setValue('customSlippage', window.slippage);
};

export const setTab = function(name) {
...
window.copyAddrText = copyAddrText;
window.copyMktAddr = copyMktAddr;
window.showSlippageModal = showSlippageModal;
window.setTab = setTab;
window.showTokenSelector = showTokenSelector;
window.hideTokenSelector = hideTokenSelector;
