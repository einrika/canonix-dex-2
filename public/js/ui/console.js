...
console.log = function(...args) { originalConsole.log.apply(console, args); addConsoleLog('log', args); };
console.warn = function(...args) { originalConsole.warn.apply(console, args); addConsoleLog('warn', args); };
console.error = function(...args) { originalConsole.error.apply(console, args); addConsoleLog('error', args); };
console.info = function(...args) { originalConsole.info.apply(console, args); addConsoleLog('log', args); };

window.logSuccess = function(...args) { originalConsole.log('✅', ...args); addConsoleLog('success', args); };
...
function formatArg(arg) {
    ...
    else if (typeof arg === 'object' && arg !== null) { try { return JSON.stringify(arg, null, 2); } catch (e) { return String(arg); } }
    return String(arg);
}
...
function addConsoleLog(type, args) {
    ...
    if (modal && !modal.classList.contains('hidden')) { renderConsoleLog(logEntry); updateConsoleStats(); } else { unreadConsoleCount++; updateConsoleBadge(); }
}
...
function toggleConsoleModal() {
    ...
    if (modal.classList.contains('hidden')) {
        modal.classList.remove('hidden'); renderAllConsoleLogs(); updateConsoleStats(); unreadConsoleCount = 0; updateConsoleBadge();
...
function updateConsoleBadge() {
    ...
    if (unreadConsoleCount > 0) {
        badge.classList.remove('hidden'); badge.textContent = unreadConsoleCount > 99 ? '99+' : unreadConsoleCount;
    } else badge.classList.add('hidden');
}

window.addEventListener('load', () => {
    console.log('🚀 SYSTEM UPLINK ESTABLISHED');
