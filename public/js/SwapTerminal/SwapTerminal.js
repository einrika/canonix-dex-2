...
import { setText, setValue, toMicroAmount } from '../core/utils.js';
import { APP_CONFIG } from '../core/config.js';

export let tradeType = 'buy';
export let feeEnabled = false;
export let slippage = 30.0;
export let gasUpdateTimeout = null;
...
export const renderSwapTerminal = async function() {
    ...
    const currentSlippage = slippage || 30.0;
    ...
    container.querySelector('#buyTab')?.addEventListener('click', () => setSwapMode('buy'));
    container.querySelector('#sellTab')?.addEventListener('click', () => setSwapMode('sell'));
    container.querySelector('#max-pay-btn')?.addEventListener('click', () => setMaxPay());
    container.querySelector('#tradePayAmount')?.addEventListener('input', () => updateTradeOutput());
    container.querySelectorAll('#percent-btns button').forEach(btn => {
        btn.addEventListener('click', () => setPercentAmount(btn.dataset.pct));
    });
    container.querySelector('#tradePercentSlider')?.addEventListener('input', (e) => setPercentAmount(e.target.value));
    container.querySelector('#reverse-pair-btn')?.addEventListener('click', () => reverseTradePair());
    container.querySelector('#show-slippage-btn')?.addEventListener('click', () => window.showSlippageModal());
    container.querySelector('#execute-swap-btn')?.addEventListener('click', () => { if (!isWatchOnly) executeTrade(); });

    (async () => {
        if (window.fetchPoolData) await window.fetchPoolData();
        if (window.updateTradeBalances) await window.updateTradeBalances();
        if (updateTradeOutput) updateTradeOutput();
    })();
};

export const setSwapMode = function(mode) {
    tradeType = mode;
    renderSwapTerminal();
};

export const reverseTradePair = function() {
    tradeType = tradeType === 'buy' ? 'sell' : 'buy';
    renderSwapTerminal();
};

export const setPercentAmount = function(val) {
    ...
    if (rawBal && val === "100") {
        ...
        const decimals = (tradeType === 'buy' || paySymbol === 'PAXI') ? 6 : (window.currentTokenInfo?.decimals || 6);
        if (tradeType === 'buy' || paySymbol === 'PAXI') {
        ...
        if ((tradeType === 'buy' || paySymbol === 'PAXI') && percent > 0.9) amount = Math.max(0, amount - 0.1);
        amountStr = amount > 0 ? amount.toFixed(6) : '';
    }
    setValue('tradePayAmount', amountStr);
    const slider = document.getElementById('tradePercentSlider');
    if (slider) slider.value = val;
    updateTradeOutput();
};

export const setMaxPay = function() { setPercentAmount(100); };

export const setSlippage = function(val) {
    slippage = val;
    setValue('customSlippage', val);
    setText('slippageVal', val.toFixed(1) + '%');
    updateTradeOutput();
};

export const updateCustomSlippage = function() {
    let val = parseFloat(document.getElementById('customSlippage')?.value);
    if (!isNaN(val) && val > 0) {
        if (val > 30) { val = 30; setValue('customSlippage', 30); }
        slippage = val;
        setText('slippageVal', val.toFixed(1) + '%');
        updateTradeOutput();
    }
...
export const updateTradeOutput = async function() {
    ...
    if (!window.poolData || payAmount <= 0 || !window.currentPRC20) {
        setValue('tradeRecvAmount', ''); setText('minRecv', '0.00');
        setText('priceImpact', '-'); setText('networkFee', '0.00 PAXI'); return;
    }
    ...
    setValue('tradeRecvAmount', outputDisplay);
    const impactEl = document.getElementById('priceImpact');
    if (impactEl) {
        setText(impactEl, priceImpact.toFixed(2) + '%');
    ...
    if (actualSlippageEl) setText(actualSlippageEl, priceImpact.toFixed(2) + '%');
    const minRecv = (outputAmount * (1 - (slippage / 100)) / Math.pow(10, targetDecimals)).toFixed(6);
    const recvSymbol = tradeType === 'buy' ? (window.currentTokenInfo?.symbol || 'TOKEN') : 'PAXI';
    setText('minRecv', `${minRecv} ${recvSymbol}`);
    ...
    if (swapBtn && !window.wallet?.isWatchOnly) {
        ...
        const amountToPayRaw = BigInt(toMicroAmount(payAmount, tradeType === 'buy' ? 6 : tokenDecimals));
        if (amountToPayRaw > balanceRaw) { swapBtn.disabled = true; swapBtn.textContent = "INSUFFICIENT BALANCE"; }
        else { swapBtn.disabled = false; swapBtn.textContent = tradeType === 'buy' ? "BUY NOW" : "SELL NOW"; }
    }
    const rate = tradeType === 'buy' ? (outputAmount / (payAmount * 1e6) * Math.pow(10, 6 - tokenDecimals)).toFixed(4) : (payAmount / (outputAmount / 1e6)).toFixed(4);
    setText('tradeRate', `1 PAXI = ${rate} ${window.currentTokenInfo?.symbol || 'TOKEN'}`);
    if (gasUpdateTimeout) clearTimeout(gasUpdateTimeout);
    gasUpdateTimeout = setTimeout(async () => {
        try {
            const gasInfo = await window.fetchGasEstimate(1);
            if (gasInfo) setText('networkFee', `${parseFloat(gasInfo.estimatedFee).toFixed(4)} PAXI (~$${gasInfo.usdValue})`);
        } catch (e) { setText('networkFee', 'Error'); }
    }, 1000);
};

export const executeTrade = async function() {
