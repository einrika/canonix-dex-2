window.UIManager.registerUI('AIModal', () => {
    return `
    ...
    content.innerHTML = `
        <div class="space-y-8 animate-fade-in">
            ...
            <div class="flex items-center justify-between">
                <div class="font-mono text-[8px] text-muted-text font-black uppercase tracking-[0.2em]">UPLINK: ${aiSource}</div>
                ${badges[aiType]}
            </div>
...
window.generateAIAnalysis = async function() {
    const content = document.getElementById('aiContent');
    if (!content) return;
    if (!window.currentPRC20) { window.renderAnalysis(null); return; }
    content.innerHTML = `
    ...
    try {
        ...
        if (serverResult) { aiText = serverResult; aiSource = 'Gemini Pro 1.5'; aiType = 'premium'; }
            ...
            aiText = window.generateAdvancedAnalysis({ symbol, name, latestPrice, change24h, sentiment, trend, rsi, macd, volatility, volatilityLabel, momentum, liquidity, liquidityScore, volume24h, volumeRatio, levels, riskLevel, riskScore, tradeSignal });
            aiSource = 'Canonix Algo'; aiType = 'manual';
        }
        window.renderAnalysis({ symbol, name, latestPrice, change24h, sentiment, trend, rsi, macd, volatility, volatilityLabel, liquidity, liquidityScore, volume24h, volumeRatio, levels, riskLevel, riskScore, tradeSignal, aiText, aiSource, aiType });
