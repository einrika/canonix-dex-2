...
<div id="faqModal" class="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm hidden">
    <div class="bg-surface border-4 border-card w-full max-w-2xl shadow-brutal animate-scale-in flex flex-col max-h-[90vh]">
        <div class="p-6 border-b-4 border-card flex items-center justify-between bg-card">
            ...
            <button id="close-faq-modal" class="w-12 h-12 border-4 border-card bg-surface text-primary-text hover:bg-meme-pink transition-colors flex items-center justify-center text-xl shadow-brutal-sm hover:shadow-none">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <div class="p-8 overflow-y-auto space-y-6">
            <div class="faq-item group">
                <button class="faq-btn w-full flex items-center justify-between p-6 bg-surface border-2 border-card hover:border-meme-cyan transition-all group shadow-brutal-sm">
...
window.UIManager.registerLogic('FAQModal', (container) => {
    container.querySelector('#close-faq-modal')?.addEventListener('click', () => window.hideFAQ());
    container.querySelectorAll('.faq-btn').forEach(btn => {
        btn.addEventListener('click', (e) => window.toggleFAQ(e.currentTarget));
    });
});
