window.UIManager.registerUI('HoldersTab', () => {
    return `
...
window.loadTokenHolders = async function(isRefresh = true) {
    ...
    try { await window.fetchTokenHolders(window.holdersPage); }
        ...
        container.innerHTML = `<div class="text-center py-20">
            ...
            <button id="reboot-scan-btn" class="px-8 py-3 bg-surface border-4 border-card text-primary-text font-display text-xl uppercase italic shadow-brutal hover:shadow-none transition-all">REBOOT SCAN</button>
        </div>`;
        document.getElementById('reboot-scan-btn')?.addEventListener('click', () => window.loadTokenHolders());
    }
};
...
window.fetchTokenHolders = async function(page) {
    ...
    try {
        const url = `${window.APP_CONFIG.BACKEND_API}/api/holders?address=${window.currentPRC20}&page=${page - 1}`;
...
window.renderHoldersTable = function(list, page) {
    ...
    if (!isEnd) {
        html += `<div id="holdersLoadMoreWrapper" class="p-10 flex justify-center bg-surface border-t-4 border-card">
            <button id="btnLoadMoreHolders" class="px-12 py-4 bg-meme-cyan text-black font-display text-3xl border-4 border-card shadow-brutal hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all uppercase italic">LOAD MORE APES</button>
        </div>`;
    } else {
    ...
    document.getElementById('btnLoadMoreHolders')?.addEventListener('click', () => window.loadMoreHolders());
};
